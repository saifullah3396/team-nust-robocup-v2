...
namespace RandomLib
{
  ...
  template<typename IntType = int, int bits = 1>
    ...
    public:
      ...
      template<class Random>
        bool
        LessThan(Random& r, IntType p, IntType q)
        {
          for (;;) {
      ...
      template<class Random>
        bool
        LessThanEqual(Random& r, IntType p, IntType q)
        {
      ...
      template<class Random>
        bool
        GreaterThan(Random& r, IntType p, IntType q)
        {
      ...
      template<class Random>
        bool
        GreaterThanEqual(Random& r, IntType p, IntType q)
        {
       ...
       **********************************************************************/
      static bool
      Check(IntType mmax, IntType qmax)
      {
        return (mmax - 1 <= ((std::numeric_limits < IntType > ::max)() >> bits) && mmax - 1 <= (std::numeric_limits < IntType > ::max)() / qmax);
      }
    private:
      IntType _a, _l; // current range is _a + [0, 2^(bits*_l)).
      template<class Random>
        static unsigned
        RandomDigit(Random& r) throw ()
        {
          return unsigned(r.template Integer<bits>());
        }
      template<class Random>
        void
        Refine(Random& r) // only gets called if _l > 0.
        {
  ...
  template<typename IntType, int bits>
    template<class Random>
      UniformInteger<IntType, bits>::UniformInteger(Random& r, IntType m,
        bool flip)
      {
        STATIC_ASSERT(
        ...
        STATIC_ASSERT(
          bits > 0 && bits < std::numeric_limits < IntType > ::digits && bits <= std::numeric_limits<
            unsigned>::digits,
  ...
  template<typename IntType, int bits>
    std::ostream&
    operator<<(std::ostream& os, const UniformInteger<IntType, bits>& u)
    {
      if (u.Entropy()) os << "[" << u.Min() << "," << u.Max() << "]";
      else os << u.Min();
      return os;
    }
