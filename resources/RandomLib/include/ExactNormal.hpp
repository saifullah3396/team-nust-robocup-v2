...
namespace RandomLib
{
  ...
  template<int bits = 1>
    ...
    private:
      ...
      template<class Random>
        bool
        ExpProb(Random& r, unsigned n) const;
  ...
  template<int bits>
    template<class Random>
      bool
      ExactNormal<bits>::ExpProbH(Random& r) const
      {
        // Bit counts
        ...
        while (true) {
          _q.Init();
          if (!_q.LessThan(r, _p)) return false;
          _p.Init();
          if (!_p.LessThan(r, _q)) return true;
        }
      }
  ...
  template<int bits>
    template<class Random>
      bool
      ExactNormal<bits>::ExpProb(Random& r, unsigned n) const
      {
        // Bit counts
  ...
  template<int bits>
    template<class Random>
      unsigned
      ExactNormal<bits>::ExpProbN(Random& r) const
      {
        // Bit counts
        // ExpProbN() = n: n * a + b
        unsigned n = 0;
              ...
              for (s = 1, first = true;; s ^= 1, first = false) {
                ...
                if (first) { // ALT: if (false) {
                  ...
                  if (y > 0) { // the y test succeeds just test q < x
                    ...
                    _p.Init();
                    if (!_x.GreaterPair(r, _q, _p)) break;
                  }
                } else {
