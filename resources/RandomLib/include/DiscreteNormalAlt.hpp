...
namespace RandomLib
{
  ...
  template<typename IntType = int, int bits = 1>
    ...
    private:
      ...
      template<class Random>
        bool
        B(Random& r, int k, IntType xn0,
          UniformInteger<IntType, bits>& j) const;
    };

  template<typename IntType, int bits>
    DiscreteNormalAlt<IntType, bits>::DiscreteNormalAlt(IntType sigma_num,
      IntType sigma_den, IntType mu_num, IntType mu_den)
    {
      ...
      if (!(mu_den / l <= maxint / sigma_num && iabs(mu_num) <= maxint / (sigma_den / l) && mu_den / l <= maxint / sigma_den)) throw RandomErr(
        "DiscreteNormalAlt: sigma or mu overflow");
      if (!UniformInteger < IntType, bits > ::Check(_isig, _d)) throw RandomErr(
      ...
      if (!(kmax + 1 <= maxint / _isig && _isig * (kmax + 1) <= maxint - iabs(
        _imu))) throw RandomErr("DiscreteNormalAlt: possible overflow a");
      ...
      if (!(_d <= maxint >> bits)) throw std::runtime_error(
        "DiscreteNormalAlt: overflow in RandomNumber a");
      if (!((IntType(1) << bits) - 1 <= maxint / _sig)) throw std::runtime_error(
    ...
    template<class Random>
      ...
      DiscreteNormalAlt<IntType, bits>::operator()(Random& r) const
      {
        for (;;) {
          ...
          IntType s = r.Boolean() ? -1 : 1, xn0 = _sig * IntType(k) + s * _mu,
            i0 = iceil(xn0, _d); // i = i0 + j
          xn0 = i0 * _d - xn0; // xn = xn0 + _d * j
          if (!j.LessThan(r, _sig - xn0, _d) || (k == 0 && s < 0 && !j.GreaterThan(
            r,
            -xn0,
            _d))) continue;
          int h = k + 1;
  ...
  template<typename IntType, int bits>
    IntType
    DiscreteNormalAlt<IntType, bits>::iceil(IntType n, IntType d)
    {
      IntType k = n / d;
      return k + (k * d < n ? 1 : 0);
    }
  ...
  template<typename IntType, int bits>
    IntType
    DiscreteNormalAlt<IntType, bits>::iabs(IntType n)
    {
      return n < 0 ? -n : n;
    }
  ...
  template<typename IntType, int bits>
    IntType
    DiscreteNormalAlt<IntType, bits>::gcd(IntType u, IntType v)
    {
  ...
  template<typename IntType, int bits>
    template<class Random>
      int
      DiscreteNormalAlt<IntType, bits>::Choose(Random& r, int m)
      {
  ...
  template<typename IntType, int bits>
    template<class Random>
      bool
      DiscreteNormalAlt<IntType, bits>::ExpProbH(Random& r) const
      {
        _p.Init();
  ...
  template<typename IntType, int bits>
    template<class Random>
      bool
      DiscreteNormalAlt<IntType, bits>::ExpProb(Random& r, int n) const
      {
        while (n-- > 0) {
  ...
  template<typename IntType, int bits>
    template<class Random>
      int
      DiscreteNormalAlt<IntType, bits>::ExpProbN(Random& r) const
      {
        int n = 0;
        while (ExpProbH(r))
          ++n;
  ...
  template<typename IntType, int bits>
    template<class Random>
        ...
        UniformInteger<IntType, bits>& j) const
      {
        int n = 0, m = 2 * k + 2, f;
        for (;; ++n) {
          if (((f = k ? 0 : Choose(r, m)) < 0) || (_q.Init(), !(
            n ? _q.LessThan(r, _p) : _q.LessThan(r, xn0, _d, _sig, j))) || ((f =
            k ? Choose(r, m) : f) < 0) || (f == 0 && (_p.Init(), !_p.LessThan(
            r,
            xn0,
            _d,
            _sig,
