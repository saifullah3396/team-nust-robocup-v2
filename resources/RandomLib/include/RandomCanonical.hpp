...
namespace RandomLib
{
  ...
  template<class Generator>
    ...
    public:
      ...
      template<typename IntType, int bits>
        ...
        Integer() throw ()
        {
          ...
          STATIC_ASSERT(
            std::numeric_limits < IntType > ::is_integer && std::numeric_limits < IntType > ::radix == 2,
            "Integer<T,b>(): bad integer type IntType");
          // Check that we have enough digits in Ran64
          STATIC_ASSERT(
            bits > 0 && bits <= std::numeric_limits < IntType > ::digits && bits <= 64,
      ...
      template<int bits>
        ...
        Integer() throw ()
        {
          return Integer<result_type, bits>();
        }
      ...
      template<typename IntType>
        IntType
        IntegerC(IntType m, IntType n) throw ();
      ///@}
      ...
      template<typename RealType, int prec>
        ...
        Fixed() throw ()
        {
          ...
          STATIC_ASSERT(
            !std::numeric_limits < RealType > ::is_integer && std::numeric_limits < RealType > ::radix == 2,
            "Fixed(): bad real type RealType");
          STATIC_ASSERT(
            prec > 0 && prec <= std::numeric_limits < RealType > ::digits,
      ...
      template<typename RealType, int prec>
        ...
        FixedS() throw ()
        {
          return Fixed<RealType, prec>() - (RealType(1) - RandomPower2::pow2<
            RealType>(-prec)) / 2;
        }
      /**
          ...
          if (prec < width) {
            ...
            while (true) {
              ...
              while (true) {
                ...
                if (Generator::Ran() >> (s > width ? 0 : width - s)) break;
                s -= width; // Decrement s
              }
            }
          }
      ...
      template<typename NumericType>
        bool
        Prob(NumericType m, NumericType n) throw ();
      ///@}
    ...
    private:
      ...
      template<typename UIntT>
        typename UIntT::type
        Unsigned(typename UIntT::type n) throw ();
      ...
      template<typename RealType, int prec, int erange, bool up>
        RealType
        FloatZ(int b, result_type m) throw ();
      ...
      template<typename RealType>
        bool
        ProbF(RealType z) throw ();
      /**
       * The two-argument version of Prob for real types
       **********************************************************************/
      template<typename RealType>
        bool
        ProbF(RealType x, RealType y) throw ();
    };
  ...
  template<class Generator>
      ...
      Generator(n)
    {
      ...
      STATIC_ASSERT(
        ...
        "RandomCanonical: illegal floating type");
      STATIC_ASSERT(
        0 <= std::numeric_limits<float>::digits && std::numeric_limits<float>::digits <= std::numeric_limits<
          double>::digits,
        "RandomCanonical: inconsistent floating precision");
#endif
#if HAVE_LONG_DOUBLE
      ...
      STATIC_ASSERT(
        2 * std::numeric_limits<unsigned long>::digits - width >= 0,
  ...
  template<class Generator>
    template<typename IntType>
      inline IntType
      RandomCanonical<Generator>::Integer() throw ()
      {
        ...
        const int d =
          std::numeric_limits < IntType > ::digits + std::numeric_limits < IntType > ::is_signed; // Include the sign bit
        // Check that we have enough digits in Ran64
        STATIC_ASSERT(d > 0 && d <= 64, "Integer: bad bit-size");
        if (d <= width) return IntType(Generator::Ran());
        else // d <= 64
    ...
    template<typename UIntT>
      ...
      RandomCanonical<Generator>::Unsigned(typename UIntT::type n) throw ()
      {
        ...
        // results are bogus, but there is no error condition.)
          r1 =
            ((UIntT::width == 32 ?
              typename UIntT::type(u32::mask) : typename UIntT::type(u64::mask)) - n) / n1,
  ...
  template<class Generator>
    template<typename IntType>
      inline IntType
      RandomCanonical<Generator>::Integer(IntType n) throw ()
      {
  ...
  template<class Generator>
    template<typename IntType>
      inline IntType
      RandomCanonical<Generator>::IntegerC(IntType n) throw ()
      {
  ...
  template<class Generator>
    template<typename IntType>
      inline IntType
      RandomCanonical<Generator>::IntegerC(IntType m, IntType n) throw ()
      {
        ...
        const int d =
          std::numeric_limits < IntType > ::digits + std::numeric_limits < IntType > ::is_signed; // Include sign bit
        // Check that we have enough digits in Ran64
  ...
  template<class Generator>
    template<typename RealType, int prec, int erange, bool up>
      inline RealType
      RandomCanonical<Generator>::FloatZ(int b, result_type m) throw ()
      {
        ...
        STATIC_ASSERT(
          !RANDOMLIB_HASDENORM(RealType) ||
          // Need 1/2^(erange+prec) > 0
          prec + erange <= std::numeric_limits < RealType > ::digits - std::numeric_limits < RealType > ::min_exponent,
        ...
        if (erange == 0 || (up ? x > RealType(0.5) : x >= RealType(0.5))) return x;
        ...
        do { // s = 2 thru erange+1, sm = erange thru 1
          x /= 2;
          if (m & 1u) return x; // x in {1, 2} / 2^s, prob 1/2^s
          if (--b) m >>= 1;
          else {
            m = Generator::Ran();
            b = width;
        ...
        x >= (RealType(1) + RandomPower2::pow2<RealType>(
          erange + std::numeric_limits < RealType > ::min_exponent)) * (
        ...
        return x - (
          erange + 1 > -RandomPower2::minpow ?
            std::pow(RealType(2), -(erange + 1) / 2) * std::pow(
              RealType(2),
              -(erange + 1) + (erange + 1) / 2) :
            RandomPower2::pow2<RealType>(-erange - 1));
        else return up ? // Underflow to up ? min() : 0
          // pow is OK here.
          std::pow(
            RealType(2),
            std::numeric_limits < RealType > ::min_exponent - 1) :
          RealType(0);
      }
  ...
  template<class Generator>
    template<typename IntType>
      inline bool
      RandomCanonical<Generator>::Prob(IntType n) throw ()
      {
        STATIC_ASSERT(
  ...
  template<class Generator>
    template<typename RealType>
      inline bool
      RandomCanonical<Generator>::ProbF(RealType p) throw ()
      {
        ...
        const int c =
          std::numeric_limits < RealType > ::digits > width ?
        ...
        return false;
        else if (p >= RealType(1)) return true;
        do { // Loop executed slightly more than once.
          ...
          if (p <= RealType(0)) return false; // If p' <= 0 the result is definitely false.
  ...
  template<class Generator>
    template<typename IntType>
      inline bool
      RandomCanonical<Generator>::Prob(IntType m, IntType n) throw ()
      {
        STATIC_ASSERT(
          std::numeric_limits < IntType > ::is_integer,
  ...
  template<class Generator>
    template<typename RealType>
      inline bool
      RandomCanonical<Generator>::ProbF(RealType x, RealType y) throw ()
      {
        STATIC_ASSERT(
          !std::numeric_limits < RealType > ::is_integer && std::numeric_limits < RealType > ::radix == 2,
        ...
        while (s > 0) { // With prob 1 - 2^-s return false
          // Check the next min(s, width) bits.
          if (Generator::Ran() >> (s > width ? 0 : width - s)) return false;
          s -= width;
        }
        // Here with prob 2^-s
        const int c =
        ...
        while (true) { // Loop executed 1/y on average
          ...
          while (true) { // Loop executed slightly more than once
            ...
            if (ya < RealType(1)) {
              ya *= mult; // Form ya'
              ya -= d;
              if (ya <= RealType(0)) break; // z > y, start again
            }
            if (xa > RealType(0)) {
              xa *= mult; // Form xa'
              xa -= d;
              if (xa >= RealType(1)) return true; // z < x
            }
            if (xa <= RealType(0) && ya >= RealType(1)) return false; // x < z < y
          }
        }
      }
  ...
  template<class Generator>
    template<int nbits>
      inline std::bitset<nbits>
      RandomCanonical<Generator>::Bits() throw ()
      {
        ...
        while (m > 0) {
          ...
          if (m < nbits) b <<= (width > ulbits ? width - ulbits : width);
...
namespace std
{
  ...
  template<class Generator>
    void
    swap(RandomLib::RandomCanonical<Generator>& r,
