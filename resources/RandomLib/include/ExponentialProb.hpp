...
namespace RandomLib
{
  ...
  public:
    ...
    template<typename RealType, class Random>
      bool
      operator()(Random& r, RealType p) const;
  ...
  private:
    ...
    template<typename RealType, class Random>
      bool
      ExpFraction(Random& r, RealType p) const;
    /**
  ...
  template<typename RealType, class Random>
    bool
    ExponentialProb::operator()(Random& r, RealType p) const
    {
      STATIC_ASSERT(
      ...
      // exp(a+b) = exp(a) * exp(b)
      ExpFraction(r, p < RealType(1) ? p : RealType(1)) && (p <= RealType(1) || operator()(
        r,
        p - RealType(1))));
    }
  ...
  template<typename RealType, class Random>
    bool
    ExponentialProb::ExpFraction(Random& r, RealType p) const
    {
      ...
      static const int c = // The Intel compiler needs this to be static??
        std::numeric_limits < word > ::digits < std::numeric_limits < RealType > ::digits ?
      ...
      if (p < RealType(1)) while (true) {
        if (p <= RealType(0)) return true;
        // p in (0, 1)
        if (l == m) _v.resize(l += alloc_incr);
        _v[m++] = r.template Integer<word, c>();
        p *= std::pow(RealType(2), c); // p in (0, 2^c)
        word w = word(p); // w in [0, 2^c)
        if (_v[m - 1] > w) return true;
        else if (_v[m - 1] < w) break;
        else // _v[m - 1] == w
      ...
      for (unsigned s = 0;; s ^= 1) { // Parity of loop count
        for (unsigned j = 0;; ++j) {
          ...
          if (w > _v[j]) return s != 0u; // New V is bigger, so exit
          else if (w < _v[j]) {
