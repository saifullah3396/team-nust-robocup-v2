...
namespace RandomLib
{
  ...
  template<int bits = 1>
    ...
    public:
      ...
      template<class Random>
        bool
        LessThan(Random& r, RandomNumber& t)
        {
      ...
      template<class Random>
        bool
        GreaterPair(Random& r, RandomNumber& u, RandomNumber& v)
        {
      ...
      template<class Random, typename IntType>
        bool
        LessThan(Random& r, IntType p0, IntType c, IntType q,
       ...
       **********************************************************************/
      const unsigned&
      RawDigit(unsigned k) const throw ()
      {
      ...
      template<typename RealType, typename Random>
        ...
        Fraction(Random& r)
        {
          ...
          for (int i = 0; i < k; ++i) {
            mult *= fact;
            y += mult * RealType(
              i < k - 1 ? RawDigit(i) : RawDigit(i) & (~0U << (k * bits - d)));
          }
          return y;
        }
      /**
      ...
      template<typename RealType, class Random>
        ...
        Value(Random& r)
        {
          ...
          if (trail > 0) {
            ...
            if (_n & (1U << (trail - 1))) y += std::pow(RealType(2), trail);
          } else {
            y = RealType(_n);
    ...
    private:
       ...
       **********************************************************************/
      static int
      highest_bit_idx(unsigned x) throw ()
      {
        if (x == 0) return 0;
        int r = 1;
        if (x & 0xffff0000U) {
  ...
  template<int bits>
    std::ostream&
    operator<<(std::ostream& os, const RandomNumber<bits>& n)
    {
