...
namespace RandomLib
{
  ...
  template<typename NumericType = double>
    ...
    public:
      ...
      template<typename WeightType>
        void
        Init(const std::vector<WeightType>& w)
        {
          Init(w.begin(), w.end());
        }
      ...
      template<class Random>
        unsigned
        operator()(Random& r) const throw ()
        {
          if (_k <= 1) return 0; // Special cases
          ...
          return
            (std::numeric_limits < NumericType > ::is_integer ?
              r.template Prob<NumericType>(
                NumericType(_Q[K]),
                NumericType(_wsum)) :
              r.template Prob<NumericType>(NumericType(_Q[K]))) ? K : _Y[K];
        }
       ...
       **********************************************************************/
      NumericType
      Weight(unsigned i) const throw ()
      {
        if (i >= _k) return NumericType(0);
        else if (_k == 1) return _wsum;
        const NumericType n =
          std::numeric_limits < NumericType > ::is_integer ?
            _wsum : NumericType(1);
        NumericType p = _Q[i];
  ...
  template<typename NumericType>
    template<typename InputIterator>
      RandomSelect<NumericType>::RandomSelect(InputIterator a, InputIterator b)
      {
        ...
        typedef typename std::iterator_traits<InputIterator>::value_type WeightType;
        // Disallow WeightType = real, NumericType = integer
        STATIC_ASSERT(
          std::numeric_limits < WeightType > ::is_integer || !std::numeric_limits < NumericType > ::is_integer,
        ...
        // as WeightType
        STATIC_ASSERT(
          std::numeric_limits < WeightType > ::is_integer != std::numeric_limits < NumericType > ::is_integer || std::numeric_limits < NumericType > ::digits >= std::numeric_limits < WeightType > ::digits,
        ...
        for (InputIterator wptr = a; wptr != b; ++wptr) {
          ...
          if (w > (std::numeric_limits < NumericType > ::max)() - _wsum) throw RandomErr(
        ...
        _k = unsigned(p.size());
        if (_wsum <= 0) throw RandomErr("RandomSelect: Zero total weight");
        ...
        if ((std::numeric_limits < NumericType > ::max)() / NumericType(_k) < NumericType(
          _wmax)) throw RandomErr("RandomSelect: Overflow");
