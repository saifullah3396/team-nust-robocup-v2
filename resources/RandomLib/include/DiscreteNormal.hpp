...
namespace RandomLib
{
  ...
  template<typename IntType = int>
    ...
    public:
       ...
       **********************************************************************/
      DiscreteNormal(IntType sigma_num, IntType sigma_den = 1, IntType mu_num =
        0, IntType mu_den = 1);
      /**
       * Return a sample.
       *
    ...
    private:
      ...
      template<class Random>
        static int
        Choose(Random& r, int m);

      // Compute v' < v.  If true set v = v'.
      template<class Random>
        bool
        less_than(Random& r) const;

      // Compute v < (x + p/_sig)/base (updating v)
      template<class Random>
        bool
        less_than(Random& r, word x, IntType p) const;

      // true with prob (x + p/_sig)/base
      template<class Random>
        bool
        bernoulli(Random& r, word x, IntType p) const;
      ...
      template<class Random>
        bool
        B(Random& r, int k, word x0, IntType xn) const;
    };

  template<typename IntType>
    DiscreteNormal<IntType>::DiscreteNormal(IntType sigma_num,
      IntType sigma_den, IntType mu_num, IntType mu_den) :
      _v(std::vector < word > (alloc_incr)), _m(0), _l(alloc_incr)
    {
      ...
      STATIC_ASSERT(
        std::numeric_limits < IntType > ::digits + 1 >= std::numeric_limits < word > ::digits,
        "DiscreteNormal: IntType must be at least as wide as word");
      if (!(sigma_num > 0 && sigma_den > 0 && mu_den > 0)) throw RandomErr(
        "DiscreteNormal: need sigma > 0");
      _imu = mu_num / mu_den;
      if (_imu == (std::numeric_limits < IntType > ::min)()) throw RandomErr(
      ...
      if (!(mu_den / l <= maxint / sigma_num && mu_num <= maxint / (sigma_den / l) && mu_den / l <= maxint / sigma_den)) throw RandomErr(
      ...
      if (!(kmax + 1 <= maxint / _isig && _isig * (kmax + 1) <= maxint - iabs(
        _imu))) throw RandomErr("DiscreteNormal: possible overflow a");
      // Check xn0 = _sig * k + s * _mu;
      if (!(kmax <= maxint / _sig && _sig * kmax <= maxint - iabs(_mu))) throw RandomErr(
        "DiscreteNormal: possible overflow b");
      // Check for overflow in LeadingDigit
      // p << bits, p = _sig - 1, bits = 8
      if (!(_sig <= (maxint >> 8))) throw RandomErr(
  ...
  template<typename IntType>
    template<class Random>
      IntType
      DiscreteNormal<IntType>::operator()(Random& r) const
      {
        for (;;) {
          int k = ExpProbN(r);
          if (!ExpProb(r, k * (k - 1))) continue;
          IntType s = r.Boolean() ? -1 : 1, xn = _sig * IntType(k) + s * _mu,
  ...
  template<typename IntType>
    IntType
    DiscreteNormal<IntType>::iceil(IntType n, IntType d)
    {
      IntType k = n / d;
      return k + (k * d < n ? 1 : 0);
    }
  ...
  template<typename IntType>
    IntType
    DiscreteNormal<IntType>::iabs(IntType n)
    {
      return n < 0 ? -n : n;
    }
  ...
  template<typename IntType>
    IntType
    DiscreteNormal<IntType>::gcd(IntType u, IntType v)
    {
  ...
  template<typename IntType>
    ...
    DiscreteNormal<IntType>::LeadingDigit(IntType& p) const
    {
      ...
      STATIC_ASSERT(
        bits * num == std::numeric_limits < word > ::digits,
  ...
  template<typename IntType>
    template<class Random>
      int
      DiscreteNormal<IntType>::Choose(Random& r, int m)
      {
  ...
  template<typename IntType>
    template<class Random>
      bool
      DiscreteNormal<IntType>::less_than(Random& r) const
      {
        for (unsigned j = 0;; ++j) {
          if (j == _m) {
          ...
          if (w > _v[j]) return false; // New V is bigger, so exit
          else if (w < _v[j]) {
  ...
  template<typename IntType>
    template<class Random>
      bool
      DiscreteNormal<IntType>::less_than(Random& r, word x, IntType p) const
      {
  ...
  template<typename IntType>
    template<class Random>
      bool
      DiscreteNormal<IntType>::bernoulli(Random& r, word x, IntType p) const
      {
        word w = r.template Integer<word>();
        if (w != x) return w < x;
        for (;;) {
  ...
  template<typename IntType>
    template<class Random>
      bool
      DiscreteNormal<IntType>::ExpProbH(Random& r) const
      {
        static const word half =
          word(1) << (std::numeric_limits < word > ::digits - 1);
        _m = 0;
  ...
  template<typename IntType>
    template<class Random>
      bool
      DiscreteNormal<IntType>::ExpProb(Random& r, int n) const
      {
        while (n-- > 0) {
          if (!ExpProbH(r)) return false;
        }
        return true;
      }
  ...
  template<typename IntType>
    template<class Random>
      int
      DiscreteNormal<IntType>::ExpProbN(Random& r) const
      {
        int n = 0;
        while (ExpProbH(r))
          ++n;
  ...
  template<typename IntType>
    template<class Random>
      bool
      DiscreteNormal<IntType>::B(Random& r, int k, word x0, IntType xn) const
      {
        int n = 0, h = 2 * k + 2, f;
        _m = 0;
        for (;; ++n) {
          if (((f = k ? 0 : Choose(r, h)) < 0) || !(
            n ? less_than(r) : less_than(r, x0, xn)) || ((f =
            k ? Choose(r, h) : f) < 0) || (f == 0 && !bernoulli(r, x0, xn))) break;
        }
        return (n % 2) == 0;
      }
