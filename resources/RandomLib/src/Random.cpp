...
namespace RandomLib
{
  ...
  template<>
    void
    Random_u32::Write32(std::ostream& os, bool bin, int& cnt,
      Random_u32::type x)
    {
      if (bin) {
        unsigned char buf[4];
  ...
  template<>
    void
    Random_u32::Read32(std::istream& is, bool bin, Random_u32::type& x)
    {
      if (bin) {
        unsigned char buf[4];
        ...
        // Use network order -- most significant byte first
        x =
          Random_u32::type(buf[0]) << 24 | Random_u32::type(buf[1]) << 16 | Random_u32::type(
            buf[2]) << 8 | Random_u32::type(buf[3]);
      } else {
        std::string s;
        is >> std::ws >> s;
        // Use hexadecimal to minimize storage together with stringstream to
  ...
  template<>
    void
    Random_u64::Write32(std::ostream& os, bool bin, int& cnt,
      Random_u64::type x)
    {
  ...
  template<>
    void
    Random_u64::Read32(std::istream& is, bool bin, Random_u64::type& x)
    {
      Random_u32::type t;
  ...
  RandomSeed::seed_type
  RandomSeed::SeedWord()
  {
    // Check that the assumptions made about the capabilities of the number
    // system are valid.
    STATIC_ASSERT(
      std::numeric_limits<seed_type>::radix == 2 && !std::numeric_limits<seed_type>::is_signed && std::numeric_limits<seed_type>::digits >= 32,
...
#if !RANDOMLIB_WINDOWS
      timeval tv;
      if (gettimeofday(&tv, 0) == 0) v.push_back(seed_t::cast(tv.tv_usec));
#else
      LARGE_INTEGER taux;
  ...
  template<class Algorithm, class Mixer>
    void
    RandomEngine<Algorithm, Mixer>::Init() throw ()
    {
      // On exit we have _ptr == N.

      STATIC_ASSERT(
        std::numeric_limits<typename mixer_t::type>::radix == 2 && !std::numeric_limits<typename mixer_t::type>::is_signed && std::numeric_limits<typename mixer_t::type>::digits >= int(mixer_t::width),
        "mixer_type is a bad type");

      STATIC_ASSERT(
        std::numeric_limits<result_type>::radix == 2 && !std::numeric_limits<result_type>::is_signed && std::numeric_limits<result_type>::digits >= width,
        "engine_type is a bad type");

      STATIC_ASSERT(
        mixer_t::width == 32 || mixer_t::width == 64,
      ...
      STATIC_ASSERT(
        !(mixer_t::width == width) || sizeof(_stateu) == sizeof(_state),
      ...
      if (mixer_t::width < width) {
        for (size_t i = 0; i < N; ++i)
          ...
          _state[i] = result_type(_stateu[2 * i]) | result_type(
            _stateu[2 * i + 1])
  ...
  template<class Algorithm, class Mixer>
    ...
    RandomEngine<Algorithm, Mixer>::Check(u64::type v, u32::type e,
      u32::type m) const
    {
      if (v != version) throw RandomErr(Name() + ": Unknown version");
      if (e != Algorithm::version) throw RandomErr(
        Name() + ": Algorithm mismatch");
      if (m != Mixer::version) throw RandomErr(Name() + ": Mixer mismatch");
      u32::type check = 0;
      ...
      for (std::vector<seed_type>::const_iterator n = _seed.begin();
        n != _seed.end(); ++n) {
        if (*n != seed_t::cast(*n)) throw RandomErr(
          Name() + ": Illegal seed value");
        u32::CheckSum(u32::type(*n), check);
      }
      u32::CheckSum(_ptr, check);
      if (_stride == 0 || _stride > UNINIT / 2) throw RandomErr(
        Name() + ": Invalid stride");
      u32::CheckSum(_stride, check);
      if (_ptr != UNINIT) {
        if (_ptr >= N + _stride) throw RandomErr(Name() + ": Invalid pointer");
  ...
  template<typename Algorithm, typename Mixer>
    RandomEngine<Algorithm, Mixer>::RandomEngine(std::istream& is, bool bin)
    {
      ...
      for (std::vector<seed_type>::iterator n = _seed.begin(); n != _seed.end();
        ++n) {
        u32::Read32(is, bin, t);
        *n = seed_type(t);
      }
      ...
      u32::Read32(is, bin, t);
      if (t != Check(versionr, versione, versionm)) throw RandomErr(
        Name() + ": Checksum failure");
    }

  template<typename Algorithm, typename Mixer>
    void
    RandomEngine<Algorithm, Mixer>::Save(std::ostream& os, bool bin) const
    {
  ...
  template<typename Algorithm, typename Mixer>
    void
    RandomEngine<Algorithm, Mixer>::StepCount(long long n) throw ()
    {
      ...
      if (nrounds != _rounds) Algorithm::Transition(nrounds - _rounds, _statev);
      _rounds = nrounds;
      _ptr = nptr;
    }
  ...
  template<typename Algorithm, typename Mixer>
    void
    RandomEngine<Algorithm, Mixer>::SelfTest()
    {
      RandomEngine g(std::vector < seed_type > (0));
      g.SetCount(10000 - 1);
      result_type x = g();
      if (SelfTestResult(0) && x != SelfTestResult(1)) throw RandomErr(
        Name() + ": Incorrect result with seed " + g.SeedString());
      seed_type s[] =
      ...
      }
      {
        ...
        x = h();
        if (SelfTestResult(0) && x != SelfTestResult(2)) throw RandomErr(
          Name() + ": Incorrect result with seed " + h.SeedString());
        g.SetCount(1000000);
        if (h != g) throw RandomErr(Name() + ": Comparison failure");
      }
    }

  template<>
    Random_u32::type
    RandomEngine<MT19937<Random_u32>, MixerMT0<Random_u32> >::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 : i == 1 ? 4123659995UL : 3016432305UL;
    }

  template<>
    Random_u64::type
    RandomEngine<MT19937<Random_u64>, MixerMT0<Random_u64> >::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 :
  ...
  template<>
    Random_u32::type
    RandomEngine<MT19937<Random_u32>, MixerMT1<Random_u32> >::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 : i == 1 ? 4123659995UL : 2924523180UL;
    }

  template<>
    Random_u64::type
    RandomEngine<MT19937<Random_u64>, MixerMT1<Random_u64> >::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 :
  ...
  template<>
    Random_u32::type
    RandomEngine<MT19937<Random_u32>, MixerSFMT>::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 : i == 1 ? 666528879UL : 2183745132UL;
    }

  template<>
    Random_u64::type
    RandomEngine<MT19937<Random_u64>, MixerSFMT>::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 :
  ...
  template<>
    Random_u32::type
    RandomEngine<SFMT19937<Random_u32>, MixerSFMT>::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 : i == 1 ? 2695024307UL : 782200760UL;
    }

  template<>
    Random_u64::type
    RandomEngine<SFMT19937<Random_u64>, MixerSFMT>::SelfTestResult(
      unsigned i) throw ()
    {
      return i == 0 ? 1 :
  ...
  template<class RandomType>
    void
    MixerMT0<RandomType>::SeedToState(
      const std::vector<RandomSeed::seed_type>& seed, mixer_type state[],
      unsigned n) throw ()
    {
      // Adapted from
      ...
      if (s > 0) {
        const unsigned m = mixer_t::width / 32, s2 = (s + m - 1) / m;
        unsigned i1 = 1;
        r = state[0];
        for (unsigned k = (n > s2 ? n : s2), j = 0; k;
          --k, i1 = i1 == n - 1 ? 1 : i1 + 1, // i1 = i1 + 1 mod n - 1
          j = j == s2 - 1 ? 0 : j + 1) { // j = j+1 mod s2
          r = state[i1] ^ c * (r ^ r >> (w - 2));
          r += j + mixer_type(seed[m * j]) + (
            m == 1 || 2 * j + 1 == s ?
              mixer_type(0) : mixer_type(seed[m * j + 1]) << (w - 32));
          r &= mask;
          state[i1] = r;
        }
        for (unsigned k = n - 1; k; --k, i1 = i1 == n - 1 ? 1 : i1 + 1) { // i1 = i1 + 1 mod n - 1
  ...
  template<class RandomType>
    void
    MixerMT1<RandomType>::SeedToState(
      const std::vector<RandomSeed::seed_type>& seed, mixer_type state[],
      unsigned n) throw ()
    {
      ...
      if (s > 0) {
        ...
        unsigned i1 = 0;
        for (unsigned k = (n > s2 ? n : s2), j = 0; k;
          --k, i1 = i1 == n - 1 ? 0 : i1 + 1, // i1 = i1 + 1 mod n
          j = j == s2 - 1 ? 0 : j + 1) { // j = j+1 mod s2
          r = state[i1] ^ c * (r ^ r >> (w - 2));
          r += j + mixer_type(seed[m * j]) + (
            m == 1 || 2 * j + 1 == s ?
        ...
        for (unsigned k = n; k; --k, i1 = i1 == n - 1 ? 0 : i1 + 1) { // i1 = i1 + 1 mod n
          r = state[i1] ^ d * (r ^ r >> (w - 2));
          r -= i1;
          r &= mask;
          state[i1] = r;
  ...
  void
  MixerSFMT::SeedToState(const std::vector<RandomSeed::seed_type>& seed,
    mixer_type state[], unsigned n) throw ()
  {
    ...
    for (unsigned j = 0; j < count;
      ++j, i = i == n - 1 ? 0 : i + 1, k = k == n - 1 ? 0 : k + 1, l =
        l == n - 1 ? 0 : l + 1) {
...
#define MT19937_REVSTEP(I, J, K) {                                  \
  ...
  template<class RandomType>
    void
    MT19937<RandomType>::Transition(long long count,
      internal_type statev[]) throw ()
    {
      if (count > 0) for (; count; --count) {
        // This ONLY uses high bit of statev[0]
        unsigned i = 0;
        for (; i < N - M; ++i)
          MT19937_STEP(i, i + 1, i + M);
        for (; i < N - 1; ++i)
          MT19937_STEP(i, i + 1, i + M - N);
        MT19937_STEP(N - 1, 0, M - 1);
        // i = N - 1
      }
      else if (count < 0) for (; count; ++count) {
        // This ONLY uses high bit of statev[0]
        engine_type p = statev[0];
        // Fix low bits of statev[0] and compute y[-1]
        MT19937_REVSTEP(0, N - 1, M - 1);
        // i = N
        unsigned i = N - 1;
        for (; i > N - M; --i)
          MT19937_REVSTEP(i, i - 1, i + M - 1 - N);
        for (; i; --i)
          MT19937_REVSTEP(i, i - 1, i + M - 1);
        MT19937_REVSTEP(0, N - 1, M - 1);
        // i = 0
      }
    }
...
#undef MT19937_REVSTEP
  ...
  template<class RandomType>
    void
    MT19937<RandomType>::NormalizeState(engine_type state[]) throw ()
    {
      ...
      while (i < N && state[i] == 0)
        ++i;
      if (i >= N) state[0] = engine_type(1) << (width - 1); // with prob 2^-19937
  ...
  template<class RandomType>
    void
    MT19937<RandomType>::CheckState(const engine_type state[],
      Random_u32::type& check)
    {
      ...
      if (x == 0) throw RandomErr("MT19937: All-zero state");
      ...
      engine_type q = state[N - 1] ^ state[M - 1], s = q >> (width - 1);
      q = (q ^ (s ? magic : engine_type(0))) << 1 | s;
      if ((q ^ state[0]) & lower) throw RandomErr("MT19937: Invalid state");
...
#define SFMT19937_REVSTEP32(I, J, K, L) {                   \
  ...
  template<>
    void
    SFMT19937<Random_u32>::Transition(long long count,
      internal_type statev[]) throw ()
    {
      if (count > 0) {
        // x[i+N] = g(x[i], x[i+M], x[i+N-2], x[i,N-1])
        internal_type s0 = statev[N - 8], s1 = statev[N - 7],
          s2 = statev[N - 6], s3 = statev[N - 5], r0 = statev[N - 4], r1 =
            statev[N - 3], r2 = statev[N - 2], r3 = statev[N - 1];
        for (; count; --count) {
          unsigned i = 0;
          for (; i + M < N; i += R)
            SFMT19937_STEP32(i, i + M);
          for (; i < N; i += R)
            SFMT19937_STEP32(i, i + M - N);
        }
      } else if (count < 0) for (; count; ++count) {
        unsigned i = N;
        for (; i + M > N;) {
          i -= R;
          SFMT19937_REVSTEP32(i, i + M - N, i - 2 * R, i - R);
        }
        for (; i > 2 * R;) {
          i -= R;
          SFMT19937_REVSTEP32(i, i + M, i - 2 * R, i - R);
        }
        SFMT19937_REVSTEP32(R, M + R, N - R, 0);
        // i = R
        SFMT19937_REVSTEP32(0, M, N - 2 * R, N - R);
        // i = 0
      }
    }
...
#define SFMT19937_REVSTEP64(I, J, K, L) {                   \
  ...
  template<>
    void
    SFMT19937<Random_u64>::Transition(long long count,
      internal_type statev[]) throw ()
    {
      // x[i+N] = g(x[i], x[i+M], x[i+N-2], x[i,N-1])
      if (count > 0) {
        internal_type s0 = statev[N - 4], s1 = statev[N - 3],
          r0 = statev[N - 2], r1 = statev[N - 1];
        for (; count; --count) {
          unsigned i = 0;
          for (; i + M < N; i += R)
            SFMT19937_STEP64(i, i + M);
          for (; i < N; i += R)
            SFMT19937_STEP64(i, i + M - N);
        }
      } else if (count < 0) for (; count; ++count) {
        unsigned i = N;
        for (; i + M > N;) {
          i -= R;
          SFMT19937_REVSTEP64(i, i + M - N, i - 2 * R, i - R);
        }
        for (; i > 2 * R;) {
          i -= R;
          SFMT19937_REVSTEP64(i, i + M, i - 2 * R, i - R);
        }
        SFMT19937_REVSTEP64(R, M + R, N - R, 0);
        // i = R
        SFMT19937_REVSTEP64(0, M, N - 2 * R, N - R);
        // i = 0
      }
    }
...
#undef SFMT19937_REVSTEP64
  ...
  template<>
    void
    SFMT19937<Random_u32>::NormalizeState(engine_type state[]) throw ()
    {
      // Carry out the Period Certification for SFMT19937
      engine_type inner =
        (state[0] & PARITY0) ^ (state[1] & PARITY1) ^ (state[2] & PARITY2) ^ (state[3] & PARITY3);
      for (unsigned s = 16; s; s >>= 1)
        inner ^= inner >> s;
      STATIC_ASSERT(
        PARITY_LSB < 32 && PARITY0 & 1u << PARITY_LSB,
  ...
  template<>
    void
    SFMT19937<Random_u64>::NormalizeState(engine_type state[]) throw ()
    {
      // Carry out the Period Certification for SFMT19937
      ...
      for (unsigned s = 32; s; s >>= 1)
        inner ^= inner >> s;
      STATIC_ASSERT(
        PARITY_LSB < 64 && PARITY0 & 1u << PARITY_LSB,
        "inconsistent PARITY_LSB or PARITY0");
      // Now inner & 1 is the parity of the number of 1 bits in w_0 & p.
      if ((inner & 1u) == 0)
      // Change bit of w_0 corresponding to LSB of PARITY
  ...
  template<class RandomType>
    void
    SFMT19937<RandomType>::CheckState(const engine_type state[],
      Random_u32::type& check)
    {
      engine_type x = 0;
      Random_u32::type c = check;
      ...
      if (x == 0) throw RandomErr("SFMT19937: All-zero state");
      check = c;
    }
  ...
  template<>
    RandomCanonical<MRandomGenerator32> RandomCanonical<MRandomGenerator32>::Global =
      RandomCanonical();
  template<>
    RandomCanonical<MRandomGenerator64> RandomCanonical<MRandomGenerator64>::Global =
      RandomCanonical();
  template<>
    RandomCanonical<SRandomGenerator32> RandomCanonical<SRandomGenerator32>::Global =
      RandomCanonical();
  template<>
    RandomCanonical<SRandomGenerator64> RandomCanonical<SRandomGenerator64>::Global =
      RandomCanonical();
