...
#define TORAD  3.14f/180.0f
  ...
  bool
  isResolutionValid(const int resIndex);
  ...
  void
  setSizeFromResolution(const int resIndex, int& outWidth, int& outHeight);
  ...
  int
  getResolutionFromSize(const int width, const int height);

  /*! @brief Utility function that checks that a color space index is valid.
   * (ie, present in the definitions just above)*/
  bool
  isColorSpaceValid(const int colorspace);
   ...
   */
  int
  getNumLayersInColorSpace(const int colorSpace);
   ...
   */
  int
  getNumChannelsInColorSpace(const int colorSpace);

  inline float
  convertAngleValToNormalizedImgVal(const float& radValue,
    const float& imageRadMin, const float& imageRadMax)
  {
    return (radValue / (imageRadMax - imageRadMin));
  }

  inline float
  convertAnglePosToNormalizedImgPos(const float& radPosition,
    const float& imageRadMin, const float& imageRadMax)
  {
