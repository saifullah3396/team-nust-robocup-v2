...
template<typename T>
  ...
  ImuDataFilter<T>::initiate()
  {
    ...
    T roll = atan2(acc[1], acc[2]);
    T pitch = atan2(-acc[0], acc[2]);
    Matrix<T, 3, 1> rpy(roll, pitch, 0);
    Quaternion<T> q = eulerToQuaternion(rpy);
    x.setZero();
    x[0] = q.w();
    x.segment(1, 3) = q.vec();
...
template<typename T>
  void
  ImuDataFilter<T>::measurement(Matrix<T, 3, 1>& zhat,
    Matrix<T, Dynamic, Dynamic>& H)
  {
    ...
    Quaternion<T> accQ = q.inverse() * gnQ * q; //r_n to r_b
    zhat = accQ.vec();
