...
void SpeedWalk<Scalar>::update()
{
  ...
  /*Matrix<Scalar, 3, 1> desComVelocity;
  desComVelocity[0] =
    controllers[0]->step(
      Eigen::Map<Eigen::Matrix<Scalar, Eigen::Dynamic, 1> >(
        zmpRef->x.linearize(), zmpRef->x.size()))[1];
  desComVelocity[1] =
    controllers[1]->step(
      Eigen::Map<Eigen::Matrix<Scalar, Eigen::Dynamic, 1> >(
        zmpRef->y.linearize(), zmpRef->y.size()))[1];
  desComVelocity[2] = 0.0;*/
  //Matrix<Scalar, 4, 4> target = this->kM->getGlobalToOther();
  /* Using ComIk
   *
  ...
  if (stepTraj.rows() > 0) {
    ...
    Matrix<Scalar, 4, 4> currStepT = this->kM->getGlobalToOther();
    Matrix<Scalar, 6, 1> currStepConfig;
    currStepConfig.head(3) = currStepT.block(0, 3, 3, 1);
    currStepConfig.tail(3) = MathsUtils::matToEuler(currStepT);
    ...
    /* Using ComIkboost::static_pointer_cast<CartesianTask<Scalar> >(tasks[toUType(IkTasks::step)])->setTarget(stepTarget);*/

    /***if (getBehaviorCast()->addHipCompensation && nSteps > 0) {
      hcTargetJoints = this->kM->getJointPositions();
      ///< Goes from zero to hipOffset and then back to zero
      if (currentBase == RobotFeet::lFoot) {
        hcTargetJoints[toUType(Joints::lHipRoll)] =
          hipInitPos + maxHipLOffset * sin(M_PI * (stepTrajIndex + 1) * this->cycleTime / params->totalStepTime);
        ...
        hcTargetJoints[toUType(Joints::rHipRoll)] =
          hipInitPos - maxHipROffset * sin(M_PI * (stepTrajIndex + 1) * this->cycleTime / params->totalStepTime);
  ...
  /* Using ComIkboost::static_pointer_cast<ComTask<Scalar> >(tasks[toUType(IkTasks::com)])->setTargetCom(desComPosition);
  ...
  LOG_INFO("desComVelocity6:" << desComVelocity6.transpose())
  LOG_INFO("currentBase:" << toUType(currentBase))
  ...
  LOG_INFO("cmdJoints:" << cmdJoints.transpose());
