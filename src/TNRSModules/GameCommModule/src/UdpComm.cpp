...
bool
UdpComm::resolve(const char* addrStr, int port, struct sockaddr_in* addr)
{
...
bool
UdpComm::setTarget(const char* addrStr, int port)
{
...
bool
UdpComm::setBlocking(bool block)
{
  if (block) return fcntl(sock, F_SETFL, 0) != -1;
  else return fcntl(sock, F_SETFL, O_NONBLOCK) != -1;
}

bool
UdpComm::setLoopback(bool yesno)
{
  char val = yesno ? 1 : 0;
...
bool
UdpComm::setBroadcast(bool enable)
{
  int yes = enable ? 1 : 0;
  if (setsockopt(
    sock,
...
bool
UdpComm::bind(const char* addr_str, int port)
{
...
int
UdpComm::read(char* data, int len)
{
  return ::recv(sock, data, len, 0);
}

int
UdpComm::read(char* data, int len, sockaddr_in& from)
{
...
bool
UdpComm::write(const char* data, const int len)
{
  return ::sendto(sock, data, len, 0, target, sizeof(struct sockaddr_in)) == len;
}

const char*
UdpComm::getWifiBroadcastAddress()
{
  struct ifaddrs* ifAddrStruct = 0; //nullptr;
  struct ifaddrs* ifa = 0; //nullptr;

  //determine ip address
  getifaddrs(&ifAddrStruct);
  for (ifa = ifAddrStruct; ifa != 0; ifa = ifa->ifa_next) //for(ifa = ifAddrStruct; ifa != nullptr; ifa = ifa->ifa_next)
    {
    // manpage getifaddrs    // check it is IP4
    if (ifa->ifa_addr != 0 && ifa->ifa_addr->sa_family == AF_INET) //if(ifa->ifa_addr != nullptr && ifa->ifa_addr->sa_family == AF_INET)
    {
      ...
      if (interfaceName.find("wlan") != std::string::npos) {
        ...
        inet_ntop(AF_INET, &bcast_addr, buffer, INET_ADDRSTRLEN);
        return buffer;
      }
    }
  }
