...
public:
   ...
   */
  bool
  setTarget(const char* ip, int port);
   ...
   */
  int
  read(char* data, int len, sockaddr_in& from);
   ...
   */
  bool
  write(const char* data, const int len);
...
private:
  ...
  int sock;
  bool
  resolve(const char*, int, struct sockaddr_in*);
};
