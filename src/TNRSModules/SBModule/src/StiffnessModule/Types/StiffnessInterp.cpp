...
StiffnessInterp::StiffnessInterp(
  ...
  StiffnessModule(sbModule, config, "StiffnessInterp")
{
}
...
bool StiffnessInterp::initiate()
{
  ...
  sI = JOINT_STIFFNESSES_OUT(SBModule);
  ...
  if (removeCnt == toUType(Joints::count)) {
    STIFFNESS_STATE_OUT(SBModule) = targetState;
    return false;
  }
  return true;
  #else
  ...
  for (size_t i = 0; i < toUType(Joints::count); ++i) {
    if (sToReach[i] != sToReach[i]) continue;
    sI[i] = JOINT_STIFFNESSES_OUT(SBModule)[i];
...
void StiffnessInterp::update()
{
  ...
  if (this->runTime > timeToReachS) {
    STIFFNESS_STATE_OUT(SBModule) = getBehaviorCast()->targetState;
    finish();
  }
  #else
  if (this->runTime > timeToReachS) {
