...
private:
  ...
  handleOutput()
  {
    ...
    if (teamNumber && *playerNumber && *playerNumber <= gameCtrlData.playersPerTeam && (gameCtrlData.teams[0].teamNumber == teamNumber || gameCtrlData.teams[1].teamNumber == teamNumber)) {
      const TeamInfo& team = gameCtrlData.teams[
        gameCtrlData.teams[0].teamNumber == teamNumber ? 0 : 1];
      if (gameCtrlData.state != previousState || gameCtrlData.secondaryState != previousSecondaryState || gameCtrlData.kickOffTeam != previousKickOffTeam || team.teamColour != previousTeamColour || team.players[*playerNumber - 1].penalty != previousPenalty) {
        ...
        if (team.players[*playerNumber - 1].penalty != PENALTY_NONE) setLED(
          chestRed,
          1.f,
          0.f,
          0.f);
    ...
    if (teamNumber && *playerNumber) {
      ...
      if (gameCtrlData.teams[0].teamNumber != teamNumber && gameCtrlData.teams[1].teamNumber != teamNumber) {
        ...
        if (!gameCtrlData.playersPerTeam) gameCtrlData.playersPerTeam =
          (uint8_t) * playerNumber; // we don't know better
        publish();
      }
...
public:
    ...
    ALModule(pBroker, "GameCtrl"), proxy(0), memory(0), udp(0), teamNumber(0)
  {
    ...
    try {
      ...
      if (memory->getDataList("GameCtrl/teamColour").empty()) memory->insertData(
        "GameCtrl/teamColour",
        TEAM_BLACK);
