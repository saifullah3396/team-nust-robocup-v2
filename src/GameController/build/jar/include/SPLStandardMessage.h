...
struct SPLStandardMessage
{
  ...
  // constructor
  SPLStandardMessage() :
    version(SPL_STANDARD_MESSAGE_STRUCT_VERSION), playerNum(-1), teamNum(-1),
      fallen(-1), ballAge(-1.f), intention(-1), averageWalkSpeed(-1),
      maxKickDistance(-1), currentPositionConfidence(-1),
      currentSideConfidence(-1), numOfDataBytes(0)
  {
